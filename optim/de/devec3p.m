function [bestmem,bestval,iter] = devec3p(func,VTR,D,XVmin,XVmax,NP,itermax,F,CR,strategy,refresh)
%function [bestmem,bestval,iter] = devec3p(func,VTR,D,XVmin,XVmax,NP,itermax,F,CR,strategy,refresh)
%
...
%
% Modificatins for the Thin Film Toolbox: 
% - the cost function must now be a function handle 
% - vectorized the initialization of the population array 
% - make input argument check readable 
% - each parameter has its own range [XVmin,XVmax] 
% - the cost function is a function of only one variable 
% - variable 'y' is removed. 
% - parameters are constrained to positive values
...
% ---------------
%
% func           function handle of a function f(x,y) to minimize;
...
% 	refresh = -1; 
%
% Cost function:  	function result = f(x);
...
% 	XVmin = -2; 
% 	XVmax =  2; 
% 	[bestmem,bestval,iter] = devec3(@f,VTR,D,XVmin,XVmax);
%
...
if nargin < 1 
   error('devec3: missing arguments');
else
   if ~isa(func, 'function_handle')
      error('devec3: first argument must be a function handle');
   end
end
if nargin < 2 
   VTR = 1.e-6; 
   ...
   if length(VTR)~=1
      error('devec3: argument VTR must be a scalar.');
   end 
end
if nargin < 3 
   D = 2; 
   ...
   if length(D)~=1
      error('devec3: argument D must be a scalar.');
   end
end
if nargin < 4 
   XVmin = -2*ones(D,1);
   ...
   if length(XVmin)~=D
      error('devec3: length(XVmin) must be equal to length(D).');
   end
end
if nargin < 5 
   XVmax = 2*ones(D,1); 
   ...
   if length(XVmax)~=D
      error('devec3: length(XVmax) must be equal to length(D).');
   end
end
if nargin < 6 
   NP = 10*D; 
   ...
   if length(NP)~=1 
      error('devec3: argument NP must be a scalar.');
   end
end
if nargin < 7 
   itermax = 200; 
else
   if length(itermax)~=1 || itermax < 0 
      error('devec3: argument ''itermax'' must be a positive scalar.');
   end
end
if nargin < 8 
   F = 0.8; 
   ...
   if length(F)~=1 
      error('devec3: argument F must be a scalar.');
   end
end
if nargin < 9 
   CR = 0.5; 
   ...
   if length(CR)~=1
      error('devec3: argument CR must be a scalar.');
   end
end
if nargin < 10 
   strategy = 7; 
   ...
   if length(strategy)~=1 
      error('devec3: argument ''strategy'' must be a scalar.');
   end
end
if nargin < 11 
   refresh = -1; 
   ...
   if length(refresh)~=1
      error('devec3: argument ''refresh'' must be a scalar.');
   end
end

if (NP < 5)
   NP=5;
   fprintf('devec3 >>>  NP increased to minimal value 5\n');
end
if ((CR < 0) | (CR > 1))
   CR=0.5;
   fprintf('devec3 >>>  CR should be from interval [0,1]; set to default value 0.5\n');
end
...
pop = repmat(XVmin',NP,1) + bsxfun(@times, rand(NP,D), (XVmax-XVmin)');
...
while (iter < itermax) & (bestval > VTR)
  ...
  repmat(bestmemit, NP, 1);       % population filled with previous best member
...
%-----Constrain ui to values >= 0 (brutish but works ...)---------------------
  ui(ui<0) = 0;

%-----Select which vectors are allowed to enter the new population------------
  for i=1:NP
    tempval = func(ui(i,:));   % check cost of competitor
...
%----Output section----------------------------------------------------------
  ...
  if (refresh > 0)
    if (rem(iter,refresh) == 0)
       fprintf(1,'Iteration: %d,  Best: %g,  F: %f,  CR: %f,  NP: %d\n',iter,bestval,F,CR,NP);
       for n=1:D
         fprintf(1,'d(%d) = %f\n',n,bestmem(n));
       end
    end
